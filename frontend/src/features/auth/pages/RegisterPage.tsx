...
export function RegisterPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    if (!passwordValidation.isValid) {
      setError(`Password requirements: ${passwordValidation.requirements.join(", ")}`);
      return;
    }
  ...
  const handleGitHubSignUp = async () => {
    ...
    try {
      ...
      console.log('GitHub registration successful, navigating to dashboard');
      navigate("/dashboard", { replace: true });
    } catch (err: any) {
              ...
              {formData.password && (
                <div className="mt-2 p-2 bg-gray-50 rounded-md">
                  {(() => {
                    ...
                    return (
                      <>
                        <p className={`text-xs font-medium ${validation.isValid ? 'text-green-600' : 'text-orange-600'}`}>
                          {validation.isValid ? '✓ Password meets requirements' : 'Password must be at least 6 characters long'}
                        </p>
                      </>
                    );
                  })()}
